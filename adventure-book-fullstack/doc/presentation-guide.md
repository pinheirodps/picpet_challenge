...
| :-- | :-- | :-- |
| Backend | 94 | Unit, slice, and one full-application integration test |
| Frontend unit | 67 | Components and services, Karma + Jasmine |
...
time without interfering — each game is its own row and the backend is stateless, which was
verified. But `GET /api/games` returns everyone's in-progress games, so one reader's
"Continue Playing" list shows another's game and resuming takes it over. `/admin` is likewise
open to anyone.

Deliberately not half-solved. The smallest honest fix is a `playerId` column filled from an
id the browser generates — that separates players without authenticating them. Real accounts
are what `/admin` would need.

**No optimistic locking on `GameSession`.** Two simultaneous choices on one game could
overwrite each other. A `@Version` column is the fix.
